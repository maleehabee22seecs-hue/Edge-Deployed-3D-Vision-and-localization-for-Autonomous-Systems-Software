...
class RosMotorController:
    ...
    def __init__(
        ...
        self._last_sonar_reading_m = None
    ...
    def connect(self):
        ...
        self._sonar_sub = rospy.Subscriber(self._sonar_topic, Range, self._sonar_callback, queue_size=10)
        self._connected = True

    def _sonar_callback(self, msg: Range):
        with self._lock:
            self._last_sonar_reading_m = msg.range
    ...
    def get_distances(self) -> Optional[float]:
        # Return obstacle distance based on the ultrasonic sensor reading (converted from meters to cm)
        with self._lock:
            if self._last_sonar_reading_m is not None:
                return self._last_sonar_reading_m * 100.0
            return 100.0
